...
from bokeh.palettes import Spectral10, Spectral5, Category20_20, Turbo256
...
def get_features(model, train_loader, test_loader, device):
    train_X, train_y, train_images = inference(train_loader, model, device)
    test_X, test_y, test_images = inference(test_loader, model, device)
    return train_X, train_y, test_X, test_y, test_images, train_images
...
def visualize_confusion_matrix(confusion, accuracy, label_classes, name):
    ...
    plt.savefig(name+'_ConfusionMatrix.png')
    # plt.show()
    plt.close()
    plt.cla()
    plt.clf()
...
def lin_eval(train_X, train_y, test_X, test_y, name, classes, DEVICE):
    ...
    visualize_confusion_matrix(confusion_matrix, accuracy, classes, name)

    # If you want to save the confusion matrix to file: 
    # np.savez(f'{name}_ConfustionMatrix', confusion_matrix=confusion_matrix)

    # Calculate precision, recall, f1, and make a csv
    report_dict = classification_report(true_labels, predicted_labels, target_names=classes, output_dict=True)
    pd.DataFrame(report_dict).transpose().to_csv(name+'_ClassificationReport.csv', index=True)
...
def visualize_hover_images(model_name, embeddings, test_images, pred_classes, class_names=None, true_classes=None, showplot=False):
    ...
    output_file(model_name+'_InteractiveTSNE.html')
